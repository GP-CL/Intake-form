                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
                            <div className="flex items-center justify-center mb-4">
                                ...
                                <h3 className="text-lg font-semibold text-blue-900">
                                    Your Form PDF is Ready!
                                </h3>
                            </div>

                            <p className="text-sm text-gray-600 mb-4">
                                Patient ID: <span className="font-semibold">{pdfInfo.patientId}</span>
                            </p>

                            <div className="flex gap-3 justify-center flex-wrap">
                                {/* View PDF Button */}
                                <button
                                    onClick={() => window.open(`http://localhost:3000${pdfInfo.pdfUrl}`, '_blank')}
                                ...
                                {/* Download PDF Button */}
                                <a
                                    href={`http://localhost:3000${pdfInfo.pdfUrl}`}
                                ...
                                {/* Email PDF Button (optional - requires backend implementation) */}
                                <button
                                    onClick={async () => {
                                        try {
                                            await fetch(`http://localhost:3000/api/patients/${pdfInfo.patientId}/email-pdf`, {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json' },
                            ...
                            {formData.past_medical_history.includes("Others") && (
                                <div className="mt-4">
                                    ...
                                    <input
                                        type="text"
                                        value={
                                            formData.past_medical_history.find(item => item.startsWith("Others:"))?.substring(8) || ""
                                        }
                                        onChange={(e) => {
                                            const value = e.target.value.trim();
                                            setFormData((prev) => {
                                                // Keep all existing items except "Others:" entries
                                                const baseItems = prev.past_medical_history.filter(item => !item.startsWith("Others:"));
                                        ...
                                        className="w-full max-w-2xl px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            ...
                            {(formData.past_surgical_history.includes("Others") ||
                                ...
                                <div className="mt-4">
                                    ...
                                    <input
                                        type="text"
                                        value={
                                            formData.past_surgical_history.find(item => item.startsWith("Others:"))?.split(":")[1]?.trimStart() || ""
                                        }
                                        onChange={(e) => {
                                            const value = e.target.value;
                                            setFormData((prev) => {
                                                // Keep all existing items except "Others:" entries
                                                const baseItems = prev.past_surgical_history.filter(item => !item.startsWith("Others:"));
                            ...
                            <table className="w-full border border-gray-300">
                                ...
                                <tbody>
                                    {[
                                        ...
                                        { label: 'Chest', v: 'Flat', p: 'Moderate', k: 'Expanded' },
                                        { label: 'Appetite', v: 'Irregular', p: 'Strong, Unbearable', k: 'Slow but Steady' },
                                        ...
                                        { label: 'Mobility', v: 'Fast walking talking', p: 'Moderate', k: 'Loves Sitting, Sleeping, Be Static' }
